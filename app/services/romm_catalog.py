...
def platform_summaries(db) -> list[dict]:
    """Count live synced games by stable RomM platform identity."""
    rows = db.execute(
        "SELECT rr.platform_id, i.platform AS shelf_platform, "
        "COALESCE(gp.name, i.platform, rr.platform_id) AS platform_name, "
        "COUNT(*) AS game_count "
        "FROM romm_records rr JOIN items_live i ON i.id = rr.item_id "
...
def fetch_page(
    ...
    *,
    ...
    total = db.execute(
        ...
        "JOIN items_live i ON i.id = rr.item_id" + clause,
        params,
    ).fetchone()["c"]
    rows = db.execute(
