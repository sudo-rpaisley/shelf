"""Browse-facing projection of the signed-in user's media state.

The catalogue row remains shared. Consumption status, Wishlist, rating,
favourite, progress and personal notes come from ``user_item_state`` and are
projected onto mutable Browse dictionaries only for rendering/filter counts.
A missing personal row is genuinely unset; it never falls back to another
account's legacy shared state.
"""
...
def overlay_items(db, user_id: int, items: list[dict]) -> list[dict]:
    """Overlay one user's personal state onto mutable Browse item dictionaries."""
    if not items:
        return items
