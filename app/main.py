    ...
    while True:
        ...
        try:
            with get_db() as db:
                ...
                last = db.execute(
                    "SELECT value FROM settings WHERE key = 'abs_last_sync'"
                ).fetchone()
                now = time.time()
    ...
    while True:
        ...
        try:
            with get_db() as db:
                ...
                last = db.execute(
                    "SELECT value FROM settings WHERE key = 'hc_last_sync'"
                ).fetchone()
                now = time.time()
                if last and last["value"]:
                    elapsed = now - float(last["value"])
...
async def check_loan_reminders() -> bool:
    """Send one overdue-loan digest when one is due."""
    from app.database import get_setting
    ...
    with get_db() as db:
        ...
        if (
            last
            and last["value"]
            and now - float(last["value"]) < _LOAN_REMINDER_INTERVAL
        ):
            return False
...
def strip_html(value: str) -> str:
    """Render API-sourced rich text as plain text."""
    if not value:
        return ""
...
# Routers are registered explicitly here; app/routers/__init__.py stays empty.
...
app.include_router(personal_state.router)
