...
# 1. Series-row fragment permissions.
rep(
    "app/routers/series_memberships.py",
...
# 2. Global Series operations use every membership.
p = Path("app/routers/series.py")
text = p.read_text()

marker = '''UNASSIGNED_STRIP_CAP = 12


def find_gaps'''
helper = '''UNASSIGNED_STRIP_CAP = 12
...
if old not in text:
    raise SystemExit("check-series local query anchor not found")
text = text.replace(old, new, 1)
...
new = '''    with get_db() as db:
        ...
        _upsert_series_complete(db, name, value)'''
if old not in text:
    raise SystemExit("complete-series count anchor not found")
text = text.replace(old, new, 1)

rename_start = text.index("    with get_db() as db:\n", text.index("async def rename_series"))
rename_end_line = '        return {"ok": True, "name": new_name, "merged": merged, "count": count}\n'
rename_end = text.index(rename_end_line, rename_start) + len(rename_end_line)
rename_body = '''    with get_db() as db:
        _sync_item_series_memberships(db)
...
'''
text = text[:rename_start] + rename_body + text[rename_end:]

remove_start = text.index("    with get_db() as db:\n", text.index("async def remove_all_from_series"))
remove_end_line = '        return {"ok": True, "count": count}\n'
remove_end = text.index(remove_end_line, remove_start) + len(remove_end_line)
remove_body = '''    with get_db() as db:
        _sync_item_series_memberships(db)
        rows = db.execute(
...
'''
text = text[:remove_start] + remove_body + text[remove_end:]
p.write_text(text)

# 3. Generic wording on the global Series surface.
p = Path("app/templates/series.html")
text = p.read_text()
for old, new in [
    ("Remove all books&hellip;", "Remove all items&hellip;"),
    ("Both series' books will be combined under", "Both series' items will be combined under"),
    ("Remove all books from this series?", "Remove all items from this series?"),
    ("The books stay in your library.", "The items stay in your library."),
    (">Remove books</span>", ">Remove items</span>"),
]:
    if old not in text:
        raise SystemExit(f"series template wording anchor not found: {old}")
    text = text.replace(old, new, 1)
p.write_text(text)

# 4. Connected-item tests.
for path, pairs in {
    "tests/test_abs_links.py": [
    ...
    "tests/test_abs_public_url.py": [
        ('assert "Also in Audiobookshelf (Audiobook)" in html',
         'assert "Available in" in html\n    assert "Audiobookshelf" in html'),
    ],
    "tests/test_romm_public_url.py": [
    ...
    for old, new in pairs:
        if old not in text:
            raise SystemExit(f"connected assertion anchor not found in {path}: {old}")
        text = text.replace(old, new, 1)
    p.write_text(text)
...
text = p.read_text()
for old, new in [
    ('assert "Platforms:" in response.text', 'assert \'data-testid="related-platforms"\' in response.text'),
    ...
    ('assert "Formats:" in response.text', 'assert \'data-testid="related-formats"\' in response.text'),
    ('assert "Add related media" in response.text', 'assert "Connect an item" in response.text'),
    ('assert "Add related media" not in detail.text', 'assert "Connect an item" not in detail.text'),
]:
    if old not in text:
        raise SystemExit(f"related-media assertion anchor not found: {old}")
    text = text.replace(old, new, 1)
p.write_text(text)

# 5. Regression coverage for global multi-series operations.
p = Path("tests/test_item_series_memberships.py")
text = p.read_text()
if "test_global_series_page_includes_secondary_memberships" in text:
    raise SystemExit("global multi-series tests already present")
text += r'''
