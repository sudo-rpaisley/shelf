...
After a sync, **Settings → Integrations → RomM → Browse synced library** opens a read-only catalogue of records contributed by RomM. It shows counts by stable RomM platform identity, supports platform filtering and title/publisher search, and paginates large libraries rather than loading every game in one response.

Games with the same title on different platforms remain separate provider records. Soft-deleted Shelf items are excluded through the normal `items_live` catalogue boundary. The catalogue does not infer that a RomM game is the same release as a physical cartridge or disc and does not create Related Media links automatically. Use explicit Related Media relationships when the user knows two records belong together.
