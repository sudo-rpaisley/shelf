...
def replace(path: str, old: str, new: str, expected: int = 1) -> None:
    ...
    if actual != expected:
        raise RuntimeError(
            f"{path}: expected {expected} occurrence(s), found {actual}: {old[:100]!r}"
        )
    p.write_text(text.replace(old, new, expected))


# ---------------------------------------------------------------------------
# Shared membership reconciliation
# ---------------------------------------------------------------------------
replace(
    "app/services/series_memberships.py",
...
# ---------------------------------------------------------------------------
# Hardcover — every book_series membership, not just the first
# ---------------------------------------------------------------------------
replace(
...
# ---------------------------------------------------------------------------
# Open Library — use its explicit edition series field, never title guessing
# ---------------------------------------------------------------------------
replace(
...
# ---------------------------------------------------------------------------
# ISBN cascade and normal item save
# ---------------------------------------------------------------------------
replace(
...
# ---------------------------------------------------------------------------
# Audiobookshelf — series[] + sequence, including multiple memberships
# ---------------------------------------------------------------------------
replace(
...
# ---------------------------------------------------------------------------
# Komga — reconcile its explicit one-series metadata immediately on updates
# ---------------------------------------------------------------------------
replace(
...
# ---------------------------------------------------------------------------
# IGDB — all franchises become explicit series memberships
# ---------------------------------------------------------------------------
replace(
    "app/services/igdb.py",
    '''    # Series / franchise
    series_name = None
...
# ---------------------------------------------------------------------------
# TMDb — belongs_to_collection becomes a Film & TV series
# ---------------------------------------------------------------------------
replace(
...
# Regression tests
# ---------------------------------------------------------------------------
Path("tests/test_automatic_series_metadata.py").write_text('''from app.routers import items_catalog
