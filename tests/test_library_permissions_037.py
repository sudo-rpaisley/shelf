...
def test_legacy_fork_ledger_prevents_revoked_membership_from_being_regranted(db):
    viewer = _user(db, "revoked-before-037", "viewer")
    db.execute(
        "INSERT INTO settings (key, value) VALUES (?, ?)",
        (
            libraries.LEGACY_FORK_LEDGER_KEY,
            json.dumps(
                [
                    {
                        "version": 62,
                        "description": "Seed Main Library memberships from existing roles",
                    }
                ],
                separators=(",", ":"),
            ),
    ...
    # This user deliberately has no membership row. Replaying the new 0.37
    # snapshot must honour that legacy state instead of inferring access again
    # from users.role.
    db.execute(_migration_sql(46))
    assert db.execute(
        "SELECT 1 FROM library_memberships WHERE library_id = 1 AND user_id = ?",
        (viewer["id"],),
    ).fetchone() is None
