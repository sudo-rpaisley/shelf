"""Personal Continue / automatic series progression on Shelf 0.37."""
...
def test_continue_never_skips_hidden_or_obviously_missing_series_members(
    ...
    # These are ordinary visible catalogue items; they are omitted only because
    # Continue refuses to leap a protected/gap boundary.
    assert viewer_client.get(f"/item/{visible_three}").status_code == 200
    assert viewer_client.get(f"/item/{gap_three}").status_code == 200


def test_continue_state_is_per_user(db, viewer_client, viewer_user, admin_user):
    first = _insert_item(
        db,
        title="Household Saga One",
        isbn=None,
        media_type="book",
        series_name="Household Saga",
        series_position=1,
    )
    second = _insert_item(
        db,
        title="Household Saga Two",
        isbn=None,
        media_type="book",
        series_name="Household Saga",
        series_position=2,
    )
    # Another account completing volume one must not create an Up Next card
    # for the signed-in viewer.
    user_state.set_reading_status(db, admin_user["id"], first, "read")
    db.commit()

    rail = viewer_client.get("/api/home/continue")
    assert rail.status_code == 200
    assert "Household Saga Two" not in rail.text

    user_state.set_reading_status(db, viewer_user["id"], first, "read")
    db.commit()
    rail = viewer_client.get("/api/home/continue")
    assert "Household Saga Two" in rail.text
    assert f'/item/{second}?from=home' in rail.text
