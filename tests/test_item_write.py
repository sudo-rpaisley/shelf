...
#: Every raw `UPDATE items SET` outside item_write.py must be explicitly
#: reviewed and named here — repo-relative path -> SET-clause substrings that
#: identify each legitimate statement in that file. User-editable catalogue
#: values should use item_write; the exceptions below are connector-owned
#: metadata, generated cover paths, cleanup IDs, migrations, or compatibility
#: projections from already-validated/canonical state. Derived from
#: `grep -rn "UPDATE items SET" app/ --include=*.py` outside item_write.py.
#: A dynamic `f"UPDATE items SET {...}"` matches nothing here and therefore
#: remains an unconditional violation.
RAW_UPDATE_ALLOWLIST: dict[str, set[str]] = {
    # Cover pipeline (download/upload/remove/sync fallback) — cover_path is
    # generated by Shelf after image storage, never typed as a path by a user.
    ...
    "app/routers/items.py": {
        "cover_path = ?",
        # Synopsis fetch backfills a provider result, not an edit-form value.
    ...
    "app/routers/hardcover.py": {
        "cover_path = ?",
        # IDs returned by Hardcover after a successful sync match.
        "hardcover_book_id = ?, hardcover_user_book_id = ?",
    },
    "app/services/audiobookshelf.py": {"cover_path = ?"},
    "app/services/archive.py": {"cover_path = ?"},
    "app/routers/magazines.py": {"cover_path = ?"},
    "app/routers/music.py": {
        "cover_path = ?",
        # MusicBrainz is authoritative for the attached release metadata;
        # media type and provider barcode are normalised/preflighted first.
        "title = ?, authors = ?, upc = ?, media_type = ?",
        "title = ?, authors = ?, publisher = ?, publish_year = ?",
    },

    # Connector sync/cleanup writes are authoritative external metadata or
    # connector identity maintenance, not form-submitted catalogue edits.
    "app/services/romm.py": {
        "title=?, media_type='digital_game'",
        "cover_path = ?",
    },
    "app/routers/romm.py": {"romm_id = NULL, romm_platform_id = NULL"},
    ...
    "app/routers/komga.py": {"komga_id = NULL, komga_library_id = NULL"},

    # Compatibility projections. The precise holding, personal state or
    # item_series rows are authoritative; these maintain legacy items columns
    # for existing filters/integrations after the source value is validated.
    "app/routers/location_tree.py": {
        "location_id = NULL WHERE location_id",
        "location_id = ? WHERE id",
    },
    "app/routers/item_copies.py": {"location_id = ? WHERE id"},
    "app/routers/shelf_fill.py": {
        "owned = 1, updated_at",
        "location_id = ?, updated_at",
    },
    "app/routers/user_state_items.py": {
        "reading_status = ?, date_started = ?, date_finished = ?",
    },
    "app/services/series_memberships.py": {
        "series_name = ?, series_position = ?",
        "series_position = ?, updated_at",
    },
    "app/routers/series.py": {
        "series_name = ?, series_position = ?",
        "series_name = NULL, series_position = NULL",
    },

    # ISBNdb price lookup for the insurance valuation report — computed value.
    "app/routers/valuation.py": {"estimated_value = ?, value_updated_at"},
    # Flat-location/platform deletion cascades clear compatibility FKs.
