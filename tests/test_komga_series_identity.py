...
Komga is the source of truth for Komga-backed Digital Comics. Shelf must keep
Komga's series title and series ID intact instead of trying to merge similarly
named source series into a new canonical Shelf series.
"""
...
def test_sync_preserves_komga_series_titles_instead_of_canonicalising(db, admin_client):
    ...
    rows = db.execute(
        "SELECT series_name, komga_series_id FROM items ORDER BY komga_id"
    ).fetchall()
...
def test_same_title_different_komga_series_ids_stay_separate(db, admin_client):
    ...
    detail = admin_client.get(
        "/series/detail?name=Shared%20Name&media_type=digital_comic&komga_series_id=series_a"
    ).text
